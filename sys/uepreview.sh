...
case "$(file --dereference --brief --mime-type -- "$1")" in
	...
	text/* | */xml | application/json | application/x-ndjson) bat -p --theme ansi --terminal-width "$(($4-3))" -f "$1" ;;
	audio/* | application/octet-stream) mediainfof "$1" || exit 1 ;;
	video/* )
		CACHE="${XDG_CACHE_HOME:-$HOME/.cache}/lf/thumb.$(stat --printf '%n\0%i\0%F\0%s\0%W\0%Y' -- "$(readlink -f "$1")" | sha256sum | cut -d' ' -f1)"
...
# if [ -f "$1" ] && file --mime-type "$1" | grep -q "image/\(png\|jpeg\|jpg\|gif\|bmp\|tiff\|webp\)"; then
#     ueberzug preview -- "$1"
# else
#     file "$1"
# fi
